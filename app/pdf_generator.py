...
def generate_pdfs(entries, image_paths, logo_path, output_dir, template, merge):
    # Themes for overall page background
    themes = {
        'summer': '#FFF8E1',
        'fall': '#FFECB3',
        'christmas': '#E1F5FE',
    ...
    for entry in entries:
        ...
        c.setFillColor(HexColor('#D9EAF7'))  # Light blue title bar
        ...
        # Product Image Row Background
        c.setFillColor(HexColor('#F0F0F0'))  # Light gray behind image
        ...
        # Left Column - Price Box (Moved down)
        c.setFillColor(HexColor('#FFC107'))
        c.rect(40, 40, 220, 50, fill=1, stroke=0)
        c.setFillColor(HexColor('#000000'))
